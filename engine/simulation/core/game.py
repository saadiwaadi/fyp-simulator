    ...
    def play(self, home_team, away_team, h_players, a_players, sys_style=None, match_seed=None):
        ...
        for minute in range(1, self.max_minutes + 1):
            try:
                ...
                # Runs every minute regardless of turnovers/blocks so the
                # presentation layer never freezes.
                motion.tick_minute(minute, state.ball)
        ...
        state.stats['movement_roster'] = motion.roster()
        state.stats['movement_frames'] = motion.frames
        state.stats['pitch_size'] = list(self.env.pitch_size)
